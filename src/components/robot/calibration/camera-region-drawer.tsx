...
const RegionListItem: React.FC<{ region: DrawRegion; index: number; onChange: (newVal: HSVRange) => void, onDelete: () => void }> = ({ region, index, onChange, onDelete }) => {
      ...
      {regions.length > 0 && (
        <div className="text-xs text-main-700 dark:text-main-300">
          ...
          <ul className="space-y-1 mt-1">
            {regions.map((r, idx) => (
              <RegionListItem key={idx} region={r} index={idx} onChange={handleChangeRegion(idx)} onDelete={handleDeleteRegion(idx)} />
            ))}
          </ul>
        </div>
      )}
